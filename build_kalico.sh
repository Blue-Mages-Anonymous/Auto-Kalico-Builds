...
main() {
    ...
    echo "Kalico build directory will be: $KALICO_BUILD_DIR"
    
    # Clone or update Kalico repository
    if [ ! -d "$KALICO_BUILD_DIR" ]; then
        echo "Cloning Kalico to $KALICO_BUILD_DIR"
        git clone "$KALICO_REPO" "$KALICO_BUILD_DIR"
    else
        echo "Updating existing Kalico repository"
        cd "$KALICO_BUILD_DIR"
        git fetch origin
    ...
    # Get latest version
    latest_version=$(get_latest_kalico_version)
    echo "Latest Kalico version: $latest_version"
